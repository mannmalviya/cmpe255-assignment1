*Accuracy, latency and cost on the Emotions Dataset for NLP. CMPE 255 Data Science, Assignment 1.*
...
![The frozen split and its class counts.](reports/figures/medium/table01.png)
...
![The training set is imbalanced 9.4 : 1, yet train, val and test carry the same class mix.](reports/figures/fig01_class_balance.png)
...
![Documents are short: median 17 words, maximum 66, so a 128-token sequence covers the corpus.](reports/figures/fig02_length_distribution.png)

![Document length carries no class signal: every class has almost the same length distribution.](reports/figures/fig03_length_by_class.png)
...
![The vocabulary is Zipfian: 49 word types cover half of all tokens, and half of all types occur once.](reports/figures/fig04_vocabulary_structure.png)

![The words that best separate each class are emotion adjectives, not topics.](reports/figures/fig05_distinctive_words.png)
...
![Even 2-word documents almost always contain an emotion cue; rare-word density falls with length.](reports/figures/fig07_outliers.png)
...
![All six classes are lexically close, because the shared “i feel …” frame dominates every class.](reports/figures/fig06_class_overlap.png)
...
![The machine every timing was measured on.](reports/figures/medium/table02.png)

Seed 42 was used throughout. Exact package versions are pinned in `requirements.lock.txt`.

### 3.4 Models

![The eight models, what each was trained and tuned on, and where it ran.](reports/figures/medium/table03.png)
...
![Per-class F1 tracks class size (ρ = +0.94); cross-validation settles three design questions.](reports/figures/fig12_classical.png)
...
![Test-split quality for every model. The decision bar is the SVM's 0.858 + 0.030 = 0.888.](reports/figures/medium/table04.png)
...
![Latency, throughput and cost for models served on CPU.](reports/figures/medium/table05.png)

**Served on GPU**

![Latency, throughput and cost for models served on GPU. ✗ marks p95 outside the 50 ms budget.](reports/figures/medium/table06.png)

### 4.3 The decision rule

![The pre-registered decision rule applied to every language-model configuration.](reports/figures/medium/table07.png)
...
![How certain the fine-tuned model's win is: paired tests, bootstrap and three training seeds.](reports/figures/medium/table08.png)
...
![Macro F1 against cost per 1,000 predictions (left) and p95 latency (right). The top row shows all models; the bottom row zooms on the contenders. The dashed line is the decision bar, the dotted line the 50 ms budget, and the vertical bars span three training seeds.](reports/figures/fig13_tradeoff.png)
...
![What moving from the linear SVM to fine-tuned DistilBERT costs and buys at 1 M messages a month.](reports/figures/medium/table09.png)
...
![Accuracy within slices for the linear SVM, fine-tuned DistilBERT and zero-shot Qwen, with 95% Wilson intervals. The dashed verticals mark each model's overall accuracy.](reports/figures/fig14_error_slices.png)
...
![Silhouette stays near zero at every k, and agreement with the labels (ARI, NMI) stays near chance.](reports/figures/fig08_clustering_metrics.png)

![KMeans clusters do not recover the emotions: every cluster mixes every label.](reports/figures/fig09_cluster_label_contingency.png)
...
![No LSH setting beats exact search: true neighbours sit at cosine 0.54, on the shoulder of the bulk.](reports/figures/fig10_lsh.png)
...
![Word pairs add nothing over single words; one adjective is enough to fix the label.](reports/figures/fig11_apriori.png)
...
![Measurement defects found during the study, and how each was resolved.](reports/figures/medium/table10.png)
